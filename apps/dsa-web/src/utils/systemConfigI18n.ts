...
const fieldDescriptionMap: Record<string, string> = {
  ...
  ANSPIRE_API_KEYS: 'Anspire Open 金鑰，支援逗號分隔多個；默認同時用於大模型閘道器和新聞檢索。',
  ...
  SEARXNG_PUBLIC_INSTANCES_ENABLED: '當未配置 SearXNG 自建例項時，自動從 searx.space 獲取公共例項並輪詢使用；設為 false 可停用該預設行為。',
  ...
  ENABLE_REALTIME_TECHNICAL_INDICATORS: '盤中分析時用即時價計算 MA5/MA10/MA20 與多頭排列（Issue #234）；關閉則用昨日收盤。',
  ...
  LITELLM_CONFIG: '高階模型路由 YAML 配置檔案路徑（高階用法）。僅在 YAML 可解析且產出 model_list 時優先於渠道與舊配置，否則會回退。',
  ...
  ANSPIRE_LLM_ENABLED: '開啟後，未配置更高優先順序 LLM 渠道或 OpenAI 相容金鑰時，ANSPIRE_API_KEYS 會自動作為 Anspire 大模型金鑰使用。',
