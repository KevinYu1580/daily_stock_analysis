...
function getPositionPriceLabel(row: PortfolioPositionItem): string {
  ...
  if (row.priceSource === 'realtime_quote') {
    return row.priceProvider ? `即時價 · ${row.priceProvider}` : '即時價';
  }
    ...
    <div className="portfolio-page min-h-screen space-y-4 p-4 md:p-6">
      <section className="space-y-3">
        <div className="space-y-2">
          <h1 className="text-xl md:text-2xl font-semibold text-foreground">持倉管理</h1>
        ...
        {hasAccounts ? (
          ...
          <InlineAlert
            ...
            message="還沒有可用賬戶，請先建立賬戶後再錄入交易或匯入 CSV。"
          />
        )}
      </section>
      ...
      {(showCreateAccount || !hasAccounts) ? (
        <Card padding="md">
          <div className="flex items-center justify-between gap-2">
            <h2 className="text-sm font-semibold text-foreground">新建賬戶</h2>
            {hasAccounts ? (
              <button
                type="button"
      ...
      <section className="grid grid-cols-1 xl:grid-cols-3 gap-3">
        <Card className="xl:col-span-2" padding="md">
          <div className="flex items-center justify-between mb-3">
            <h2 className="text-sm font-semibold text-foreground">持倉明細</h2>
            <span className="text-xs text-secondary">共 {positionRows.length} 項</span>
          </div>
          {positionRows.length === 0 ? (
            <EmptyState
              title="當前無持倉資料"
              description="錄入交易或匯入 CSV 後，這裡會展示按賬戶彙總的持倉明細。"
      ...
      {writeBlocked && hasAccounts ? (
        <InlineAlert
          ...
          message="當前處於“全部賬戶”檢視。為避免誤寫，請先選擇一個具體賬戶後再進行手工錄入或 CSV 提交。"
        />
      ) : null}
      ...
      <section className="grid grid-cols-1 xl:grid-cols-3 gap-3">
        <Card padding="md">
          ...
          <form className="space-y-2" onSubmit={handleTradeSubmit}>
            <input className={PORTFOLIO_INPUT_CLASS} placeholder="股票程式碼（例如 600519）" value={tradeForm.symbol}
        ...
        <Card padding="md">
          ...
          <form className="space-y-2" onSubmit={handleCashSubmit}>
            ...
            <input className={PORTFOLIO_INPUT_CLASS} placeholder={`幣種（可選，預設 ${writableAccount?.baseCurrency || '賬戶基準幣'}）`} value={cashForm.currency}
