...
const ChatPage: React.FC = () => {
  ...
  const downloadMessageAsMarkdown = useCallback((msg: Message) => {
    ...
    const heading = msg.role === 'user' ? '# 使用者訊息' : `# AI 回覆${skillLabel ? ` · ${skillLabel}` : ''}`;
  ...
  const renderThinkingBlock = (msg: Message) => {
    ...
    const summary = `${toolSteps.length} 個工具呼叫 · ${totalDuration.toFixed(1)}s`;
    ...
    return (
      ...
      >
        ...
        <span className="flex items-center gap-1.5">
          <span className="opacity-60">思考過程</span>
      ...
      <div className="flex h-full min-w-0 flex-1 flex-col overflow-hidden">
        <header className="mb-4 flex-shrink-0 space-y-3">
          <div className="flex items-start justify-between gap-4">
            ...
            {messages.length > 0 && (
              <div className="flex flex-shrink-0 flex-wrap items-center justify-end gap-2">
                <Tooltip content="匯出會話為 Markdown 檔案">
                  <span className="inline-flex">
                    <Button
                      variant="action-primary"
                      size="sm"
                      onClick={() => downloadSession(messages)}
                      aria-label="匯出會話為 Markdown 檔案"
                    >
                      <svg
                        className="w-4 h-4"
                        fill="none"
                ...
                <Tooltip content="傳送到已配置的通知機器人/郵箱">
                  <span className="inline-flex">
                    <Button
                      variant="action-primary"
                      size="sm"
                      ...
                      onClick={async () => {
                        ...
                        try {
                          ...
                          showSendFeedback({ type: 'success', message: '已傳送到通知渠道' }, 3000);
                        } catch (err) {
                          const parsed = getParsedApiError(err);
                          showSendFeedback({
                            type: 'error',
                            message: parsed.message || '傳送失敗',
                          }, 5000);
                        } finally {
                          setSending(false);
                        }
                      }}
                      aria-label="傳送到已配置的通知機器人/郵箱"
                    >
                      {sending ? (
                        <svg
          ...
          <p className="text-secondary-text text-sm">
            向 AI 詢問個股分析，獲取基於技能視角的交易建議與即時決策報告。
          </p>
          {sendToast ? (
            <InlineAlert
              variant={sendToast.type === 'success' ? 'success' : 'danger'}
              title={sendToast.type === 'success' ? '傳送成功' : '傳送失敗'}
          ...
          >
            {messages.length === 0 && !loading ? (
              <div className="flex h-full items-center justify-center">
                <EmptyState
                  title="開始問股"
                  description="輸入「分析 600519」或「茅臺現在能買嗎」，AI 將呼叫即時資料工具為您生成決策報告。"
            ...
            {skills.length > 0 && (
              ...
              <div className="flex items-end gap-3">
                <textarea
                  ...
                  placeholder="例如：分析 600519 / 茅臺現在適合買入嗎？ (Enter 傳送, Shift+Enter 換行)"
                  disabled={loading}
                  rows={1}
