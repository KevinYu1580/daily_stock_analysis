...
const RUNTIME_CAPABILITY_OPTIONS: Array<{ value: LLMCapabilityCheck; label: string; hint: string }> = [
  { value: 'json', label: 'JSON', hint: '檢測 response_format JSON 輸出是否可用。' },
  { value: 'tools', label: 'Tools', hint: '檢測 function/tool calling 是否可用。' },
  { value: 'stream', label: 'Stream', hint: '檢測流式輸出是否能返回有效 chunk。' },
  { value: 'vision', label: 'Vision', hint: '檢測當前模型是否接受 image_url 輸入。' },
];
      ...
      >
        ...
        <div className="min-w-0 flex-1">
          ...
          <p className="mt-0.5 truncate text-[11px] text-secondary-text">
            {modelCount > 0 ? `${modelCount} 個模型已配置` : '未配置模型'}
          </p>
        </div>

        <span className="flex shrink-0 items-center gap-2">
          {testState?.status === 'success' ? (
            <Tooltip content="連線正常">
          ...
          {testState?.status !== 'idle' ? (
            <Badge variant={statusVariant}>
              {testState?.status === 'success' ? '連線正常' : testState?.status === 'error' ? '連線失敗' : '測試中'}
            </Badge>
          ) : null}
        </span>

        <Tooltip content="刪除渠道">
          <span className="inline-flex">
            <Button
              type="button"
              variant="ghost"
        ...
        <div className="settings-surface-overlay-soft space-y-4 px-4 py-4">
          <div className="grid gap-2 sm:grid-cols-2">
            ...
            <div className="space-y-2">
              <label className="block text-sm font-medium text-foreground">協議</label>
              <Select
          ...
          {showProviderTemplateDetails ? (
            <div className="space-y-2 rounded-xl border border-[var(--settings-border)] bg-[var(--settings-surface-hover)] p-3">
              <div className="flex flex-wrap items-center gap-2">
                <span className="text-[11px] font-medium text-muted-text">配置參考</span>
          ...
          <Input
            ...
            placeholder={channel.protocol === 'ollama' ? '本地 Ollama 可留空' : '支援多個 Key 逗號分隔'}
          />
          ...
          <div className="space-y-3 rounded-xl border border-[var(--settings-border)] bg-[var(--settings-surface-hover)] p-3">
            <div className="flex flex-wrap items-center gap-2">
              ...
              >
                {discoveryState?.status === 'loading' ? '獲取中...' : '獲取模型'}
              </Button>
              ...
              >
                {discoveryState?.text || '支援 `/models` 的 OpenAI Compatible 渠道可自動拉取模型。'}
              </span>
            </div>
            {discoveryState?.hint ? (
            ...
            {discoveredModels.length > 0 ? (
              <div>
                <label className="mb-2 block text-sm font-medium text-foreground">可選模型（可多選）</label>
            ...
            <Input
              label={discoveredModels.length > 0 ? '手動模型（逗號分隔）' : '模型（逗號分隔）'}
              ...
              hint={
                discoveredModels.length > 0
                  ? '如有自定義模型名未出現在列表中，可繼續手動補充，儲存格式仍為逗號分隔。'
                  : '若渠道不支援自動發現或請求失敗，可直接手動填寫模型列表。'
              }
            />

            {manualOnlyModels.length > 0 ? (
              <p className="text-[11px] text-secondary-text">
                額外手動模型：{manualOnlyModels.join('，')}
              </p>
            ) : null}
          </div>
          ...
          <div className="flex items-center gap-2 pt-1">
            ...
            >
              {testState?.status === 'loading' ? '測試中...' : '測試連線'}
            </Button>
            {testState?.text ? (
              <div className="space-y-1">
                ...
                {selectedModels[0] ? (
                  <p className="text-[11px] text-secondary-text">
                    基礎連線測試預設使用模型列表首項：{selectedModels[0]}
                  </p>
                ) : null}
                {testState.hint ? (
                  <p className="text-[11px] text-secondary-text">
          ...
          <div className="space-y-3 rounded-xl border border-[var(--settings-border)] bg-[var(--settings-surface-hover)] p-3">
            <div className="flex flex-wrap items-center justify-between gap-2">
              <div>
                <p className="text-[11px] font-medium text-muted-text">執行時能力檢測（可選）</p>
                <p className="mt-0.5 text-[11px] text-secondary-text">
                  僅在手動觸發時發起真實 LLM 請求；多選可能需要 20-40 秒。
                </p>
              </div>
              <Button
                type="button"
...
function buildModelOptions(models: string[], selectedModel: string, autoLabel: string): Array<{ value: string; label: string }> {
  ...
  if (selectedModel && !models.includes(selectedModel)) {
    options.push({ value: selectedModel, label: `${selectedModel}（當前配置）` });
  }
...
const LLM_ERROR_LABELS: Record<string, string> = {
  ...
  request_blocked: '請求被攔截',
  empty_response: '空響應',
  format_error: '格式異常',
  network_error: '網路異常',
  invalid_config: '配置無效',
  unsupported_protocol: '協議暫不支援',
  capability_unsupported: '能力不支援',
  skipped: '已跳過',
};

const LLM_TROUBLESHOOTING_HINTS: Record<string, string> = {
  auth: '請檢查 API Key 是否正確、是否有多餘空格，以及當前渠道是否需要額外組織/專案許可權。',
  timeout: '可重試；若持續超時，請檢查 Base URL、網路代理、服務商可用區或本地防火牆。',
  quota: '請檢查餘額、套餐額度、RPM/TPM 限流或併發設定，必要時稍後重試。',
  model_not_found: '請確認模型名與渠道協議匹配，並先用“獲取模型”核對該渠道實際可用模型列表。',
  empty_response: '渠道已連通但未返回正文；可嘗試切換相容模型、關閉額外響應模式後再測試。',
  network_error: '請檢查 Base URL、代理、TLS/證書、中轉閘道器或本地網路策略，並可稍後重試。',
  invalid_config: '先補齊協議、Base URL、API Key 和模型配置，再執行一鍵測試。',
  unsupported_protocol: '當前僅對 OpenAI Compatible / DeepSeek 渠道提供自動模型發現，請改為手動維護模型列表。',
};

const LLM_REASON_HINTS: Record<string, string> = {
  missing_api_key: 'API Key 為空，或逗號分隔後沒有任何可用 Key；請填入至少一個有效 Key 後再測試。',
  api_key_rejected: '服務商拒絕了當前 API Key；請檢查 Key、組織/專案許可權、區域和賬號狀態。',
  rate_limit: '服務商觸發 RPM/TPM 或併發限流；請降低請求頻率或稍後重試。',
  insufficient_balance: '服務商返回餘額、賬單或額度不足；請檢查賬戶餘額和套餐狀態。',
  quota_exceeded: '服務商返回配額已耗盡；請確認賬號套餐、餘量和專案額度。',
  provider_blocked: '請求被服務商或中轉閘道器攔截；請檢查賬號風控、地域限制、模型許可權、代理商閘道器策略、內容安全策略或請求來源限制。',
  dns_error: '域名解析失敗；請檢查 Base URL 域名、網路代理和 DNS 配置。',
  tls_error: 'TLS/證書握手失敗；請檢查 HTTPS 證書、中轉閘道器或公司代理策略。',
  connection_refused: '目標服務拒絕連線；請確認 Base URL 埠、服務程序和防火牆配置。',
  model_access_denied: '當前賬號無法使用該模型；請確認模型是否已開通、賬號是否可見，或模型是否已被停用。',
  provider_prefix_mismatch: '模型 provider 字首與當前渠道不匹配；請確認模型名是否應使用該渠道的 OpenAI-compatible 路由。',
  capability_unsupported: '當前模型或相容層不支援該能力；這不影響基礎文本連線，可換模型或關閉該能力依賴。',
};

function getLlmStageLabel(stage?: string | null): string {
  return LLM_STAGE_LABELS[stage || ''] || '連線測試';
}

function getLlmErrorCodeLabel(code?: string | null): string {
  return LLM_ERROR_LABELS[code || ''] || '測試失敗';
}
...
function getLlmTroubleshootingHint(
  ...
  if (code === 'format_error') {
    return context === 'discovery' || stage === 'model_discovery'
      ? '該渠道返回的 /models 響應格式不相容，請改為手動填寫模型列表。'
      : '返回結構與預期不一致，請確認該渠道相容 Chat Completions 介面。';
  }
  if (code === 'empty_response' && (context === 'discovery' || stage === 'model_discovery')) {
    return '該渠道的 /models 介面未返回可用模型 ID；請檢查 Base URL 是否指向相容的模型列表介面，或改為手動填寫模型列表。';
  }
...
function buildLlmTestHint(result: {
  ...
  const modelHint = testedModel ? `本次測試模型：${testedModel}。` : '';
  const scopeInfo = '基礎連線測試預設只測試模型列表中的第一個模型。';
...
function buildLlmFailureText(result: {
  ...
  const summary = result.message || '測試失敗';
...
function summarizeCapabilityResults(results: Partial<Record<LLMCapabilityCheck, LLMCapabilityCheckResult>>): string {
  ...
  return `能力檢測完成：${passed} 通過 / ${failed} 失敗 / ${skipped} 跳過`;
}
...
export const LLMChannelEditor: React.FC<LLMChannelEditorProps> = ({
  ...
  const handleSave = async () => {
    ...
    if (hasEmptyName) {
      setSaveMessage({ type: 'local-error', text: '渠道名稱不能為空，且只能包含字母、數字或下劃線。' });
      return;
    }
    ...
    if (managesRuntimeConfig) {
      ...
      if (invalidPrimaryModel) {
        setSaveMessage({ type: 'local-error', text: '當前主模型不在已啟用渠道的模型列表中，請重新選擇。' });
        return;
      }
      ...
      if (invalidAgentPrimaryModel) {
        setSaveMessage({ type: 'local-error', text: '當前 Agent 主模型不在已啟用渠道的模型列表中，請重新選擇。' });
        return;
      }
      ...
      if (invalidFallbackModel) {
        setSaveMessage({ type: 'local-error', text: '存在無效的備選模型，請重新選擇。' });
        return;
      }
      ...
      if (invalidVisionModel) {
        setSaveMessage({ type: 'local-error', text: '當前 Vision 模型不在已啟用渠道的模型列表中，請重新選擇。' });
        return;
      }
    }
    ...
    try {
      ...
      setSaveMessage({ type: 'success', text: managesRuntimeConfig ? 'AI 配置已儲存' : '渠道配置已儲存' });
    } catch (error: unknown) {
      setSaveWarnings([]);
  ...
  const handleTest = async (channel: ChannelConfig, index: number) => {
    ...
    try {
      ...
      const text = result.success
        ? `連線成功${result.resolvedModel ? ` · ${result.resolvedModel}` : ''}${result.latencyMs ? ` · ${result.latencyMs} ms` : ''}`
      ...
      setTestStates((previous) => ({
        ...previous,
        [index]: { status: 'error', text: parsed.message || '測試失敗' },
      }));
    }
  };
    ...
    try {
      ...
      setDiscoveryStates((previous) => ({
        ...
        [channel.id]: {
          status: result.success ? 'success' : 'error',
          text: result.success
            ? `已獲取 ${result.models.length} 個模型${result.latencyMs ? ` · ${result.latencyMs} ms` : ''}`
            : buildLlmFailureText(result),
      ...
      >
        <div className="space-y-1">
          ...
          <p className="text-xs text-muted-text">
            新增服務商渠道後可自動獲取模型列表並多選，也可繼續手動填寫。配置會自動同步到 .env 檔案。
          </p>
        </div>
        <span className="text-xs text-muted-text">{isCollapsed ? '▶ 展開' : '▼ 收起'}</span>
      </button>
        ...
        <div className="space-y-4 animate-in fade-in slide-in-from-top-2 duration-300">
          <div className="rounded-[1.35rem] border border-[var(--settings-border)] bg-[var(--settings-surface)] p-4 shadow-soft-card">
            <div className="mb-3 flex items-center justify-between">
              <div>
                <h4 className="text-sm font-medium text-foreground">快速新增渠道</h4>
                <p className="mt-1 text-xs text-secondary-text">先選擇預設服務商，再一鍵建立配置草稿。</p>
              </div>
              <Badge variant="default" className="border-[var(--settings-border)] bg-[var(--settings-surface-hover)] text-muted-text">{channels.length} 個渠道</Badge>
            </div>
            <div className="flex items-center gap-2">
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between px-1">
              ...
              {channels.length > 0 ? (
                <span className="text-[10px] text-muted-text">{channels.filter((c) => c.enabled).length}/{channels.length} 已啟用</span>
              ) : null}
            </div>
            ...
            {channels.length === 0 ? (
              <div className="settings-surface-overlay-muted rounded-[1.35rem] border border-dashed settings-border-strong px-4 py-10 text-center">
                <p className="text-sm font-medium text-secondary-text">還沒有渠道</p>
                <p className="mt-1 text-xs text-muted-text">選擇服務商預設後點擊“新增渠道”即可開始配置。</p>
              </div>
          ...
          {managesRuntimeConfig ? (
            <div className="rounded-[1.35rem] border border-[var(--settings-border)] bg-[var(--settings-surface)] p-4 shadow-soft-card">
              <div className="mb-4 flex items-center justify-between">
                <div>
                  <span className="settings-accent-text text-xs font-medium uppercase tracking-wider">執行時引數</span>
                  <p className="mt-1 text-[11px] text-muted-text">主模型、備選模型、Vision 與 Temperature 會直接寫入執行時配置。</p>
                </div>
              ...
              <div className="mb-4">
                ...
                <p className="mt-1 text-[11px] text-secondary-text">
                  控制模型輸出隨機性，0 為確定性輸出，2 為最大隨機性，推薦 0.7。
                </p>
              </div>
              ...
              {availableModels.length === 0 ? (
                <div className="rounded-xl border border-dashed settings-border-strong settings-surface-overlay-soft px-3 py-2 text-xs text-muted-text">
                  先新增至少一個已啟用渠道並填寫模型，下面的主模型 / 備選模型 / Vision 選項才會出現。
                </div>
              ) : (
                <div className="space-y-4">
                  <div>
                    ...
                    <Select
                      ...
                      options={buildModelOptions(availableModels, runtimeConfig.primaryModel, '自動（使用第一個可用模型）')}
                      disabled={busy}
                      placeholder=""
                    />
                  </div>
                  ...
                  <div>
                    ...
                    <Select
                      ...
                      options={buildModelOptions(availableModels, runtimeConfig.agentPrimaryModel, '自動（繼承普通分析主模型）')}
                      disabled={busy}
                      placeholder=""
                    />
                  </div>
                  ...
                  <div>
                    <label className="mb-2 block text-xs text-muted-text">備選模型</label>
                  ...
                  <div>
                    ...
                    <Select
                      ...
                      options={buildModelOptions(availableModels, runtimeConfig.visionModel, '自動（跟隨 Vision 預設邏輯）')}
                      disabled={busy}
                      placeholder=""
                    />
                  </div>
            ...
            <InlineAlert
              variant="warning"
              message="檢測到已配置高階模型路由 YAML：此處僅管理渠道條目和基礎連線資訊。執行時主模型 / 備選模型 / Vision / Temperature 仍由下方通用欄位決定；若 YAML 解析成功，則以其中的路由與可用模型宣告為準，本配置不會覆蓋 YAML 檔案本身。"
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            >
              {isSaving ? '儲存中...' : managesRuntimeConfig ? '儲存 AI 配置' : '儲存渠道配置'}
            </Button>
            {!hasChanges ? <span className="text-xs text-muted-text">當前沒有未儲存的改動</span> : null}
          </div>
