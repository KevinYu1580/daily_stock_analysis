  ...
  const handleImageFile = useCallback(
    async (file: File) => {
      ...
      if (!IMG_EXT.includes(ext)) {
        setError('圖片僅支援 JPG、PNG、WebP、GIF');
        return;
      }
      ...
      try {
        ...
        let fallback = '識別失敗，請重試';
        if (err?.response?.status === 429) fallback = '請求過於頻繁，請稍後再試';
        else if (err?.code === 'ECONNABORTED') fallback = '請求超時，請檢查網路後重試';
  ...
  const mergeToWatchlist = useCallback(async () => {
    ...
    try {
      ...
      if (e instanceof SystemConfigConflictError) {
        await onMerged(value);
        setError('配置已更新，請再次點選「合併到自選股」');
      } else {
        setError(e instanceof Error ? e.message : '合併儲存失敗');
      }
    } finally {
      setIsMerging(false);
    }
  ...
  return (
    <div className="space-y-4">
      <div className="settings-surface-panel settings-border-strong rounded-xl border p-4 shadow-soft-card">
        <p className="text-sm font-medium text-foreground">支援圖片、CSV/Excel 檔案與剪貼簿文本</p>
      ...
      >
        ...
        <div className="flex flex-col gap-2 sm:flex-row">
          <textarea
            placeholder="或貼上 CSV/Excel 複製的文本..."
      ...
      {isLoading && <p className="text-sm text-secondary-text">處理中...</p>}
      {error && (
        <InlineAlert
          variant="danger"
          message={error}
      ...
      {items.length > 0 && (
        <div className="space-y-2">
          <InlineAlert
            variant="warning"
            message="建議人工逐條核對後再合併。高置信度預設勾選，中/低置信度需手動確認。"
            className="rounded-xl px-3 py-2 text-xs shadow-none"
          />
          <div className="flex items-center justify-between">
            <span className="text-xs text-secondary-text">
              共 {validCount} 條可合併，已勾選 {checkedCount} 條
            </span>
