...
- **籌碼分布(台股化,部分已落地)**:`FinmindFetcher.get_chip_distribution` 已改回 FinMind 台股籌碼面(三大法人買賣超 / 融資融券 / 外資持股比例),`ChipDistribution.market_type=="tw"` 走新分支;集保戶股權分散表(`taiwan_stock_holding_shares_per`,大戶/散戶持股集中度)需 FinMind 付費級別,免費 token 取不到,暫缺;美股無等價籌碼源,`get_chip_distribution` 對美股回 None。
- **台股新聞來源(已落地)**:配置 `FINMIND_TOKEN` 時 `FinMindNewsSearchProvider`(`taiwan_stock_news`)插到搜尋鏈最前,台股個股新聞優先走 FinMind;`_multi_dimension_search` 仍用 round-robin 分配各維度,台股時 `latest_news` 維度落到 FinMind,其餘維度仍走網頁引擎(可在後續 PR 改為台股各維度統一走 FinMind 並去重);美股 / 大盤新聞不受影響。
